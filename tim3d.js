...
const { Vector3, InitWindow } = require('raylib')
...
for (let i = 0; i < MAX_COLUMNS; i++) {
  ...
  positions.push(
    r.Vector3(
      r.GetRandomValue(-15, 15),
      newHeight / 2,
      r.GetRandomValue(-15, 15)
    )
  )
  colors.push(
    r.Color(r.GetRandomValue(20, 255), r.GetRandomValue(10, 55), 30, 255)
  )
}

r.SetCameraMode(camera, r.CAMERA_FIRST_PERSON) // Set a first person camera mode

r.SetTargetFPS(135) // Set our game to run at 60 frames-per-second
//--------------------------------------------------------------------------------------
